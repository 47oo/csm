...
> 的 5 个产品端点、F005 IPAddress 的 5 个产品端点，以及 F007 Container 的 5 个产品端点：
> 登记（必选绑定**恰好一个**运行载体 BareMetal 或 VirtualMachine）、查询（含按载体限定读取）、
> 可选字段维护、逻辑删除；**载体内活跃 `name` 唯一**（大小写敏感、软删释放）由两条 partial
> unique index 最终保证，「恰好一个载体」由 `CHECK (num_nonnulls(...) = 1)` 保证；
> 并落地 F014「载体（BareMetal **或** VM）有活跃 Container → 拒删载体」双载体端到端与创建侧
> `FOR SHARE` 并发协议。Container **无状态、无 `cluster_id`（归属由载体推导）、无 K8s / Docker /
> 运行时接入、无位置模型**。
...
`ix_network_interfaces_bare_metal_id`；**无任何唯一索引**），F005 的 `ip_addresses` 表
...
`ix_ip_addresses_cluster_id` / `ix_ip_addresses_network_interface_id`；**CHECK 集合为空、无 COLLATE / 无 lower() 索引**），
以及 F007 的 `containers` 表（`fk_containers_bare_metal` / `fk_containers_virtual_machine` 均 RESTRICT、
`ck_containers_carrier_exactly_one`（`num_nonnulls(bare_metal_id, virtual_machine_id) = 1`）、
`ux_containers_bare_metal_name_active` / `ux_containers_virtual_machine_name_active` 两条 partial unique、
`ix_containers_bare_metal_id` / `ix_containers_virtual_machine_id`；**无状态列 / 无 `cluster_id` / 无载体判别列**）；见
...
`docs/database/f005-ip-address-migration.md`、`docs/database/f007-container-migration.md`、
...
| `POST` | `/api/containers` | 需要 | 登记 Container（`carrier_type` + `carrier_id` + `name` 必填；`carrier_type` 为封闭枚举 `BARE_METAL` / `VIRTUAL_MACHINE`）；载体不存在 / 已删 / 类型与标识不一致 → `404`；**同一载体内**活跃同名 → `409`（`details[].code == "DUPLICATE"`）；`201` |
| `GET` | `/api/containers` | 需要 | 列出活跃 Container（分页 `page` / `page_size`，可选成对 `carrier_type` + `carrier_id`）；空集合 `200` + `items: []`；仅给其一 → `400`；载体不存在 / 已删 → `404` |
| `GET` | `/api/containers/{container_id}` | 需要 | 按 id 读取；不存在 / 已逻辑删除 → `404 NOT_FOUND` |
| `PATCH` | `/api/containers/{container_id}` | 需要 | 更新四个可选字段（`image` / `cpu` / `memory` / `owner`）；`name` / 载体绑定不可变；空 body → `400` |
| `DELETE` | `/api/containers/{container_id}` | 需要 | 逻辑删除 → `204`；不存在 / 已删 → `404`（委托统一软删服务） |
...
- 契约正文见 `docs/api/f001-cluster.md`、`docs/api/f002-bare-metal.md`、`docs/api/f006-virtual-machine.md`、`docs/api/f009-cluster-resource-view.md`、`docs/api/f013-auth.md`、
  `docs/api/f004-network-interface.md`、`docs/api/f005-ip-address.md`、`docs/api/f007-container.md`、`docs/api/f014-soft-delete.md`；通用约定见 `docs/api/api-conventions.md`。
...
- **F007 后** `containers` 端点可用：Container 必属**恰好一个**运行载体（`carrier_type` + `carrier_id`，`BARE_METAL` 或 `VIRTUAL_MACHINE`）；**无** `status`、**无** `cluster_id`（集群归属由载体推导）、**无** K8s / Docker / 运行时 / 位置字段；`name` **仅载体内活跃唯一**（大小写敏感、软删释放），由两条 partial unique index 最终保证；`GET /api/containers?carrier_type=&carrier_id=` 为按载体限定的 canonical 读取能力（供 F010 复用），载体存在但无活跃 Container → `200` 空集，载体不存在 / 已删 → `404`。
- **F007 后** `DELETE /api/bare-metals/{bare_metal_id}` 与 `DELETE /api/virtual-machines/{virtual_machine_id}` 在载体存在活跃 Container 时返回 `409 CONFLICT`（`details[].code == "ACTIVE_CHILDREN_EXIST"`）；软删全部活跃 Container 后可删。`BARE_METAL_ACTIVE_CHILD_CHECKS` **追加**活跃 Container 检查（保留 F006 活跃 VM、F004 活跃 NIC），`VIRTUAL_MACHINE_ACTIVE_CHILD_CHECKS` 由显式空元组演进为含 `has_active_containers_on_virtual_machine`；新增 `CONTAINER_ACTIVE_CHILD_CHECKS = ()`（F008 追加位置）。
...
│   ├── containers/     # F007 Container 模块：router / schemas / service / repository / deletion
...
│   ├── models/         # 每类资源一个独立模块、一张独立表（cluster / bare_metal / virtual_machine / network_interface / ip_address / container + users / sessions）
│   ├── schemas/        # Pydantic schema
│   ├── config.py       # 环境变量配置
│   └── main.py         # 应用工厂（挂载 AuthMiddleware）
└── migrations/         # Alembic（env.py + versions/0001_f012_baseline.py + 0002_f013_auth.py + 0003_f002_bare_metals.py + 0004_f006_virtual_machines.py + 0005_f004_network_interfaces.py + 0006_f005_ip_addresses.py + 0007_f007_containers.py）
...
├── test_containers_concurrency.py
├── test_containers_guards.py
...
2. **软删不占唯一性**：`ux_clusters_name_active` 的 predicate 为 `deleted_at IS NULL`，已软删行不占用唯一性（ADR-0004 / R-DELETE-006）；BareMetal `hostname` 同 Cluster 唯一、VirtualMachine `name` 全局唯一、IPAddress `ip_address` 同 Cluster 唯一与 Container `name` **载体内**唯一同理（`ux_bare_metals_cluster_hostname_active` / `ux_virtual_machines_name_active` / `ux_ip_addresses_cluster_ip_active` / `ux_containers_bare_metal_name_active` / `ux_containers_virtual_machine_name_active`）。**NetworkInterface 无已确认唯一性规则**（NQ-2），`network_interfaces` 表**不存在任何唯一索引**，也不实现应用层名称唯一性预检（同宿主同名允许多张）。**Container 载体身份 = （`carrier_type`, `carrier_id`）二元组**：跨载体类型即使数值 id 相同也可同名（两条 partial unique index 各自只覆盖一种载体列，另一列为 NULL 不参与唯一冲突），已由数据库测试固定。
