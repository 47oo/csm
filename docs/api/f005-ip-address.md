...
- **IP 地址范围段（地址池）的管理**已由独立的 F020 特征交付，权威正文为 `docs/api/f020-ip-address-range.md`；范围段是**独立资源**（独立表、独立端点），**不是** IPAddress 的字段。因此本契约**不定义、也不排除**范围段能力。本契约仍**不包含**：CIDR / 子网 / 网关 / 使用率统计 / 冲突扫描 / **IP 分配与回收工作流**（分配属 F021，当前 BLOCKED；F020 只做范围段管理，不含分配）。`ip_address` 的**自由文本登记立场不变**：范围段的引入**不新增**对 `ip_address` 的格式校验 / 归一化 / trim。
