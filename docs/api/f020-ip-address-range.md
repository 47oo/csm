...
- **`name`（未定义约束，不承诺）**：本契约**不承诺** `name` 的长度 / 首尾空白（trim）/ 空串 / 非法字符 / `/` / Unicode 归一化 / 大小写折叠行为。**唯一承诺**：同一 Cluster 内活跃范围段 `name` **字面等值唯一**、**区分大小写**（§22）；**不做** `lower(name)` 唯一索引、**不声明** `COLLATE`。含首尾空白或空字符串的 `name` 在现有规则下**不会被拒绝**——这**不得**解读为「空 `name` 合法」已确认。
