...
- 不存在 `name` / `description` / 用途 等未确认字段；
- 不存在 CIDR / 前缀长度 / 网络地址 / 广播地址 / 网关 / VLAN / DHCP / DNS 字段；
- 不存在使用率 / 剩余地址 / 容量字段；
...
- 请求 schema **封闭**（`extra="forbid"`）。**不接受** `name` / `description` / `status` / `deleted_at` / `id` / `created_at` / `updated_at` / CIDR / 前缀长度 / 用途 等任何未识别字段（→ `400`）。
...
```json
{ "start_ip": "10.0.0.1", "end_ip": "10.0.1.255" }
```

**可变字段（封闭集合）**：**恰为 `{start_ip, end_ip}`**。
...
| 情形 | HTTP | `error.code` | `details[]` |
|---|---|---|---|
| 请求体含未识别 / 不可变字段（含 `id` / `cluster_id` / `created_at` / `deleted_at` / `status` / `name`） | `400` | `VALIDATION_ERROR` | `field` = 该字段名 |
...
### 4.2 `409 CONFLICT` — 范围内存在活跃 IP（删除守卫）
...
| 情形 | HTTP | `error.code` | `details` |
|---|---|---|---|
| 字段格式 / 必填 / 未识别字段 / 空 PATCH / `null` / 非法 IPv4 / `start > end` | `400` | `VALIDATION_ERROR` | `details[].field` |
| 未认证 | `401` | `UNAUTHENTICATED` | `[]` |
...
| `201` | 登记成功 | — |
| `204` | 删除成功（无响应体） | — |
| `400` | 请求格式或字段校验失败（缺字段、非整数参数、`null`、未识别字段、空 PATCH、非法 IPv4、`start > end`） | `VALIDATION_ERROR` |
...
- 范围段的 `name` / `description` / 用途 / 负责人 / 分配对象等未确认字段。
