...
| 表（建议名） | 需要保障的行为 | 依据 |
| --- | --- | --- |
| `users` | 平台用户；`id` 稳定主键；`username`（仅字母与数字、长度 1–128、区分大小写、**创建后不可修改**）；`role` 单一角色；`status ∈ {enabled, disabled}`；`is_builtin`（内置 `admin` 受保护标记）；`must_change_password`；`version` 乐观锁；`created_at/updated_at` | §4.9.2/4/6/7/11、BQ-X、BQ-Y |
...
- **内置管理员保护（BQ-Y）**：内置默认管理员账号 `admin`（`users.is_builtin=true`）**不可删除、不可禁用、不可修改角色（恒为 admin）**；删除/禁用/改角色时若目标为内置账号，拒绝返回 `409 PROTECTED_ADMIN`，不改变任何数据。**不设通用“最后一个管理员”限制**；由内置 `admin` 保证系统至少保留一个可登录管理员。
...
- `D-LAST-ADMIN`（已裁定，BQ-Y）：**仅保护内置账号 `admin`**（不可删除/禁用/改角色，`409 PROTECTED_ADMIN`）；不设通用“最后一个管理员”限制。
