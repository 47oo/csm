...
## 与 R-CLUSTER-005 的关系（需产品注意）

R-CLUSTER-005 的技术理由是「Cluster 名称用于 URL 路径寻址」。本 ADR 采用 id 作为规范路径后，该理由**部分减弱**，但：

- 规则本身是 **CONFIRMED 产品规则**，本架构**无条件保留并强制校验**（写入路径拒绝含 `/` 的名称）；
- 方案 C 的 `by-name` 只读别名让该规则继续有实际意义；
- **是否修改该规则属于产品决策，Architect 不代为决定**，已作为 Non-blocking 问题反馈产品。
