...
**IP 分配（R-IP-005 ~ R-IP-010）**：分配**不是独立领域对象**，其唯一产物是创建一条绑定活跃 NetworkInterface 的 IPAddress；Cluster 由 `NIC → BareMetal` 受控推导，调用方不可指定。自动分配在 Cluster 全部活跃范围段**并集**内取**数值最小的未占用 IPv4**；「是否在范围内」按 IPv4 **数值**、「是否已占用 / 唯一性」按 `ip_address` **字面**（二者不得混用）；软删释放；**无隐式保留地址**；分配**不新增**超出 R-IP-001 的唯一性。
