...
**IP 地址范围段（地址池，R-IP-004）**：每个 Cluster 可设定多个 IP 地址范围段（`start_ip`–`end_ip`，IPv4，含两端）。范围段是**独立资源**，**无状态**、逻辑删除；**同一 Cluster 内的活跃范围段不得重叠**，跨 Cluster 允许相同范围；范围内仍有活跃 IP 时禁止删除该范围段。

范围段另有 **3 个可选元数据字段**（2026-09-21 `DEC-024` 裁定，修订 R-IP-004）：`name`（网段自定义名称，**同一 Cluster 内活跃范围段唯一**、区分大小写、软删释放、跨 Cluster 可重复，长度 / trim / 空串未定义）；`subnet_mask`（dotted-quad IPv4 **合法掩码**，二进制连续 1 后连续 0，**不强制**与 start–end 自洽，V1 仅 IPv4）；`vlan`（整数 `1`–`4094`，**不唯一**）。三者均**可选**，新增它们**不改变**重叠 / 软删 / 删除守卫 / 无状态 / IPv4 规范化，也不改变 F021 分配行为。

**IP 分配（R-IP-005 ~ R-IP-010）**：分配**不是独立领域对象**，其唯一产物是创建一条绑定活跃 NetworkInterface 的 IPAddress；Cluster 由 `NIC → BareMetal` 受控推导，调用方不可指定。自动分配**必须显式指定一个活跃范围段**（请求字段 `ip_address_range_id`），在**所选单个活跃范围段**内取**数值最小的未占用 IPv4**（不再取全部范围段的并集，耗尽也不回退）；「是否在范围内」按 IPv4 **数值**、「是否已占用 / 唯一性」按 `ip_address` **字面**（二者不得混用）；软删释放；**无隐式保留地址**；分配**不新增**超出 R-IP-001 的唯一性。
...
* IP 地址 → 网络接口：必选（2026-09-15 用户裁定；见 §5.7）；
* 虚拟机 → 裸金属：必选（R-VM-005）；
* 容器 → 虚拟机 / 裸金属：必选且恰好一个运行载体（R-CONTAINER-002）；登记粒度与生命周期见 §5.4（R-CONTAINER-001、R-CONTAINER-005）；
...
* 同一 Cluster 内的活跃 IP 地址范围段**不重叠**（R-IP-004），跨 Cluster 可重复；范围段 `name` 在**同一 Cluster 内活跃唯一**（区分大小写、软删释放）；`vlan` **不唯一**；IP 分配**不新增**超出 R-IP-001 的唯一性（R-IP-010）；
...
IP 地址范围段采用逻辑删除；范围内存在活跃 IP 时禁止删除（R-IP-004）。已分配 IP 的释放唯一途径是对该 IPAddress 逻辑删除（R-DELETE-006），**无独立回收流程**（R-IP-005 ~ R-IP-010）。
...
* 一个裸金属只能属于一种业务用途；
* 一个 IP 地址在任何场景下都全局唯一；
* 「IPv4 数值相等」等同于「`ip_address` 字面相等」——范围归属按**数值**、唯一性 / 占用按**字面**（R-IP-007）；
* 一个服务只能属于一个集群；
* 所有实时状态都需要永久保存；
* 虚拟化平台必须在第一版接入；
* CSM 必须具备传统 CMDB 的全部能力。

`requirements.md` §29 原待确认项 OPEN-005（Excel 部分成功导入策略）已于 2026-09-15 裁定为全或无（All-or-Nothing），固化为 R-IMPORT-004；不再是待确认项。
