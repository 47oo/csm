...
- **⚠️ 唯一会使其转为 Blocking 的条件**：若用户认为**「登记后变更绑定（追加 / 解除 / 替换载体）」必须属于 F008**（R-SVC-009 中「需先解除绑定或删除 Service」的措辞是支持该读法的依据），则「解除最后一个绑定」立刻成为「两种明显不同的用户行为」（拒绝 vs 允许），NQ-01 立即升级为 **Blocking**，AC-48 / AC-49 与端点范围须**重做后再进入 Architecture**。

---

## Open Questions

- **NQ-01（最高优先级，范围敏感）绑定变更与零载体**：UNCONFIRMED。范围：登记后**追加 / 解除 / 替换**运行载体；含「解除全部绑定 → 零载体」是否允许。
  - **默认（本次采纳，依据项目先例）**：不提供；绑定在登记时确定、登记后不可变（AC-48）。R-SVC-009 的「需先解除绑定或删除 Service」由「删除 Service」这一支达成（AC-39）。
  - **影响**：若纳入，将新增端点与 AC，且**必须**裁定「解除最后一个绑定」是被拒绝（建议 `409` / `400`）还是被允许（等价于承认零载体活跃 Service 合法，需同步修改 R-SVC-005 的表述）。
  - **建议**：作为独立 Feature 交付，并在其 Product 阶段先行确认 R-SVC-005 是否允许零载体；**本次不实现、不预留端点、不预留字段**。**不得**由 Architecture 或实现方自行裁定。
...
- **A-1（范围假设，最重要）**：运行载体绑定在登记时一次确定、登记后不可变；因此 F008 不提供解绑路径，零载体活跃 Service 不可由产品路径产生。依据：无 CONFIRMED 规则要求绑定变更；F002 / F006 / F007 对「绑定登记后可变性」均默认不提供。**若该假设被推翻，NQ-01 立即成为 Blocking。**
...
- **PROPOSED-3**：绑定变更（追加 / 解除 / 替换载体）应作为**独立 Feature** 设计，并在其 Product 阶段先确认 R-SVC-005 在「零载体」上的确切含义。（见 NQ-01）
