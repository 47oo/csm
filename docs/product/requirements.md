...
>
> - **2026-09-22 — 自动分配指定 IP 地址范围段阶段产品裁定（修订 2 条规则）**
>   - **DEC-025 关闭（RESOLVED，2026-09-22）**：修订 **R-IP-006** 与 **R-IP-009**（§12）。
>   - 固化：自动分配**必须显式指定**一个**活跃**且**恰属于**目标 NIC 推导出的 Cluster 的 IP 地址范围段
>     （`POST /api/ip-addresses/allocate` 新增**必填**字段 `ip_address_range_id`）；**不再**存在「全部活跃范围段
>     并集取全局最小」的隐式行为；在**所选单个范围段**内取**数值最小**的未占用 IPv4；该范围段耗尽 →
>     `409 CONFLICT + NO_AVAILABLE_IP`，**不回退**到其它范围段、不跨 Cluster 取址，**不创建任何 IPAddress**。
>   - **手动分配不变**：`POST /api/ip-addresses/allocate-manual` 语义与本次之前一致（由输入 IP 自身决定范围）。
>   - 既有边界**全部保持不变**：占用判定按 `ip_address` **字面**、范围归属按 IPv4 **数值**（R-IP-007）、
>     R-IP-001 partial unique 为唯一性**最终权威**（R-IP-010）、**无隐式保留地址**、无部分写入、F005 登记端点不变；
>     **不新增任何唯一性**、**不新增数据库 Schema**。
...
**自动分配**必须**显式指定一个活跃 IP 地址范围段**（请求字段 `ip_address_range_id`，**必填**）；分配在**所选单个活跃范围段**内，选取**数值最小**的**未占用** IPv4。

* **必须指定范围段**：自动分配请求**必须**携带 `ip_address_range_id`；**不存在**「无范围段」的自动分配，也**不再**存在「在目标 Cluster **全部活跃范围段的并集**内取全局最小」的隐式行为（该隐式行为已由用户 2026-09-22 `DEC-025` 裁定废止）。
* **所选范围段的归属与活跃性**：所选范围段必须**活跃**（未逻辑删除），且**恰属于**目标 NIC 推导出的 Cluster（其 `cluster_id` 等于该 Cluster）；不满足则分配**被拒**（**非 5xx**），**不创建任何 IPAddress**。所选范围段**不得**跨 Cluster 使用。
* **单范围内取最小**：在**所选范围段**内取**数值最小**的未占用 IPv4；**不**取跨范围段的全局最小，**不**按范围段登记顺序 / 范围段内顺序取「第一个」，**不**回退到其它范围段。
* **耗尽**：当所选范围段内不存在任何未被占用的 IPv4 时，分配按 **R-IP-009** 处理（明确**非 500** 错误，**不回退**、不跨范围段、不跨 Cluster 取址），**不创建任何 IPAddress**。
* 选中的地址以**规范化 dotted-quad**（去前导零、无前缀长度）写入 `ip_addresses.ip_address`，与 R-IP-004 对范围字段的规范化一致。
* **不引入任何隐式保留地址**：**不**自动跳过网络地址 / 广播地址 / 网关 / 范围端点或任何其它地址；范围边界完全由用户登记的所选范围段给定。
* 「未占用」的确切含义见 R-IP-007。

来源：用户 2026-09-20 对 `DEC-023` 的裁定（第 9~10 项）；用户 2026-09-22 对 `DEC-025` 的裁定（修订：自动分配必须显式指定活跃范围段、单范围内取最小、耗尽不回退）。
...
当**所选活跃范围段**中**不存在任何未被占用的 IPv4** 时（含「该 Cluster 没有任何活跃范围段」的情形），自动分配必须返回一个**明确的、非 500** 的错误，并**不创建任何 IPAddress**。
...
来源：用户 2026-09-20 对 `DEC-023` 的裁定（第 12 项）；用户 2026-09-22 对 `DEC-025` 的裁定（修订：耗尽判定限定为**所选活跃范围段**，不回退到其它范围段）。
