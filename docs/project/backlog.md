...
|---|---|---|---|---|---|---|
| F017 | 应用外壳侧边栏导航 | E07 | P2 | **DRAFT** | F012, F013, F001, F002, F004, F005, F006, F007, F008, F010（均 DONE） | **产品 / UX 未决**：用户仅提出可行性询问，是否实施待确认（NQ-1 blocking / DEC-020 OPEN） |
...
| ID | type | 关联 | 状态 | 问题 |
|---|---|---|---|---|
| DEC-020 | product | F017 | **OPEN** | 用户是否需要把应用外壳导航由顶部横向按钮改为侧边栏？若实施，范围是否限于呈现层（不引入 vue-router / 不新增依赖 / 不改路由 URL / 不改资源页面内容 / 不做响应式主题）？ |
...
| ID | 名称 | Features | 状态 |
|---|---|---|---|
| M7 | 应用外壳侧边栏导航（post-V1，呈现层） | F017 | DRAFT — 待用户确认实施 |

## Open Questions（增量）

| ID | Feature | 问题 | 阻塞 |
|---|---|---|---|
| NQ-1 | F017 | 用户是否**确实要求**实施（还是仅在询问可行性） | **是**（直接决定 F017 是否成立） |
