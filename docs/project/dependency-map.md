...
> Last updated: 2026-09-16（依赖结构不变；F012 / F001 / F013 已 DONE 并合入 develop，据此重算 Feature 状态）
>
> 依赖结构未因已完成 Feature 而变化；仅状态变化：F012 / F001 / F013 → DONE，
> F014 / F015 → READY；其余因依赖未 DONE（BLOCKED）或 Feature 级产品问题（DRAFT）尚未 READY。
...
- **Batch 1**：F001（Cluster，已 DONE）。F013（已 DONE）、F014（READY）也可与资源线并行（均只依赖 F012）。
- **Batch 2**：
  - 资源线：F002（BareMetal）
