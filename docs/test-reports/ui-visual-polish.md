...
- **更正（同日）**：此前本文与对话中「协调器实际查看了截图」的说法不成立——本会话所用模型不支持图片输入，读取截图时图像被工具省略。当时的判断依据实为：浏览器布局探针（溢出与几何断言）、CSS 静态分析，以及 Reviewer 的静态复核。现改以**线上渲染计算样式**作为客观证据（见「部署核验」）。审美结论仍属主观评价，且**未经像素级目视确认**。
...
用户授权将修改后的前端重新打包部署到线上实例。

**部署源**：`develop @ 91a9b65`（本次改动已先 `--no-ff` 合并：父 `1a743db` + `d7c85ff`，集成树与分支一致）。工作树 clean。

**执行**：`docker compose -f docker-compose.prod.yml --env-file ~/csm-deploy/csm.env build nginx` → `up -d`。
仅重建并替换 **nginx（前端）** 容器；`app` 与 `postgres` **未被重建**（Up 14h / 23h），**未运行任何 migration，数据库未被触碰**。

**产物**：

| | 部署前 | 部署后 |
| --- | --- | --- |
| JS | `index-CjbssWWR.js` | **`index-eo271_qW.js`** |
| CSS | — | **`index-Dr_I_Ahp.css`** |

**端点与既有面**（只读）：登录 200、`/api/health` 200、`/api/clusters` 200（活跃 Cluster 仍为 id5 `C01`、id6 `N96P`）、搜索空关键字 400、无命中 200、未认证 401、`/docs` 404。

**线上计算样式**（真实浏览器读取渲染结果，非 CSS 文本匹配）：

```text
侧栏 width 224px / 背景 rgb(255,255,255)
品牌 font-size 25px / weight 750
导航项（未选中）背景 transparent / height 40px
导航项（选中）背景 rgb(239,246,255) / 文字 rgb(29,78,216) / weight 600
             box-shadow inset 3px 0 0 rgb(37,99,235)
搜索分区 背景 rgb(248,250,252) / radius 10px / padding 16px 12px
内容卡片 背景 rgb(255,255,255) / radius 12px / padding 20px / 淡阴影
表头 背景 rgb(248,250,252) / 文字 rgb(100,116,139) / 12px
--el-color-primary 覆盖为 #2563eb
body 背景 rgb(246,248,251)
```

**线上浏览器探针**：对 `http://127.0.0.1/` 运行 `frontend/scripts/check-visual-layout.cjs`，**48 项检查全部通过、无 page error、无资源写入**（仅登录、读列表/详情、搜索、打开登记弹窗后取消）。

**nginx 启动**：重建后 15s 内 healthy。此前一次重建中曾观察到官方入口脚本 `10-listen-on-ipv6-by-default.sh` 卡在 `apk manifest nginx`；本次实测该命令 0.5s 即返回，未重现，判定为构建重载下的瞬时抖动。**未修改 Dockerfile**，仅保留监测解堵手段。

已保存部署后的真实截图：`deployed-desktop.png`、`deployed-mobile.png`、`deployed-login.png`、`deployed-search-mobile.png`。

**限制**：截图未经目视确认（见上文更正）；未验证其他浏览器、完整键盘/读屏与 WCAG；IPv6 未启用（与部署前一致，非本次引入）；未压测。
