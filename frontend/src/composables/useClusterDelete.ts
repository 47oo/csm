...
/** 删除失败提示视图：由稳定错误码驱动，不解析 message（f014-soft-delete.md §4）。 */
...
export interface UseClusterDeleteOptions {
  /**
   * 资源已不在活跃集合时调用：删除成功（204），或 404 NOT_FOUND
   * （不存在 / 已被逻辑删除，两者不区分，契约 §3.1）。两种结果对前端同构：
...
export interface UseClusterDeleteReturn {
  /** 正在删除的 cluster id；null = 无进行中的删除（重复提交被拦截）。 */
  ...
  requestDelete: (clusterId: number) => Promise<void>
 ...
 */
export function useClusterDelete(options: UseClusterDeleteOptions): UseClusterDeleteReturn {
  ...
  const deleteErrorView = computed<DeleteErrorView | null>(() => {
    ...
    if (error.code === 'CONFLICT') {
      // 契约 §4.1：409 的稳定判别值为 error.code === 'CONFLICT' +
  ...
  async function requestDelete(clusterId: number): Promise<void> {
    // 提交中禁止重复提交（同一行连点或跨行并发均被拦截）。
    if (deletingId.value !== null) return
    deletingId.value = clusterId
