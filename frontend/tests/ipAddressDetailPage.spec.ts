...
import IpAddressDetailPage from '../src/pages/IpAddressDetailPage.vue'
import { setUnauthenticatedHandler } from '../src/api/http'

/**
 * IP 地址详情页测试（T-41 / AC-41）。
 *
 * 覆盖：404 独立 Not Found 态（与列表 Empty 可区分）、5 字段展示（时间不透明
 * 字符串；ip_address 字面值原样展示）、无状态展示（Q-002=B）、不展示 Cluster
 * 归属（NQ-4）、ip_address 修正入口（PATCH：仅此一字段可编辑；所属网络接口
 * 不可编辑）、修改失败按 error.code 分支（400 / 404 / 409 DUPLICATE / 401）、
 * 删除入口（204 → Not Found 态 / 409 / 404 / 提交中防重复）。
 *
 * 响应体严格按 docs/api/f005-ip-address.md 构造（§2 资源表示 / §3.3 读取 /
...
function mountDetailPage(ipAddressId = 41) {
  return mount(IpAddressDetailPage, {
    props: { ipAddressId },
...
describe('IpAddressDetailPage 状态渲染', () => {
  it('请求进行中 → Loading 态（骨架屏）', async () => {
    ...
    resolveDetail(IP_ADDRESS_A)
    await waitForUi(() => {
      expect(wrapper.attributes('data-state')).toBe('content')
    })
  })

  it('成功 → 展示全部 5 字段（契约原样值，时间不透明字符串）；无状态字段（Q-002=B）、无 Cluster 归属（NQ-4）', async () => {
    vi.stubGlobal('fetch', vi.fn(async () => jsonResponse(200, IP_ADDRESS_A)))
    ...
    expect(text).toContain('IP 地址详情')
    // 全部 5 字段：契约原样值，不做任何变换。
    expect(text).toContain('所属网络接口 ID')
    expect(text).toContain('12')
    expect(text).toContain('10.0.1.1/16')
    expect(text).toContain('2026-09-18T10:00:00Z')
    // IP 无状态（Q-002=B）：不渲染状态标签 / 状态行。
    expect(wrapper.find('[data-status]').exists()).toBe(false)
    expect(text).not.toContain('状态')
    // 不暴露 deleted_at / Cluster 归属（契约 §2：字段集合封闭，NQ-4）。
    expect(text).not.toContain('deleted_at')
    expect(text).not.toContain('cluster_id')
    expect(text).not.toContain('所属集群')
  })
...
describe('IpAddressDetailPage 编辑入口（PATCH，T-41 / AC-20~22）', () => {
  it('内容态存在「编辑」入口；表单仅 ip_address 输入，不含所属网络接口选择（不可变，契约 §3.4）', async () => {
    stubDetailFetch({ detail: () => jsonResponse(200, IP_ADDRESS_A) })
    ...
    // network_interface_id 不在 PATCH 可变集内 → 表单不提供选择。
    expect(wrapper.find('.ip-address-form__nic-select').exists()).toBe(false)
    // ip_address 输入在表单中，且初值为当前记录的字面值（原样，不变换）；
    // el-input 的非 prop 属性（data-testid）落在内部 input 元素上。
    expect(wrapper.find('[data-testid="ip-form-ip-address"]').exists()).toBe(true)
    expect(
      (wrapper.find('[data-testid="ip-form-ip-address"]').element as HTMLInputElement).value,
    ).toBe('10.0.1.1/16')
  })

  it('修改字面值 → PATCH /api/ip-addresses/{id}：body 恰为 {ip_address}（可变字段封闭，契约 §3.4）', async () => {
    const fetchMock = stubDetailFetch({
      detail: () => jsonResponse(200, IP_ADDRESS_A),
      patch: () =>
    ...
    await wrapper.find('[data-testid="ip-form-ip-address"]').setValue('10.0.1.2/16')
  ...
  it('保存成功（200）→ 关闭对话框并重新读取，展示新值；network_interface_id / created_at 不变', async () => {
    const updated = {
      ...IP_ADDRESS_A,
      ip_address: '2001:db8::1',
      updated_at: '2026-09-18T12:00:00Z',
    }
    const mutable = { detail: () => jsonResponse(200, IP_ADDRESS_A) }
    const fetchMock = stubDetailFetch({
    ...
    await wrapper.find('[data-testid="ip-form-ip-address"]').setValue('2001:db8::1')
    await wrapper.find('[data-testid="ip-form-submit"]').trigger('click')

    // 等待重新读取完成：新 updated_at 只会出现在重新读取后的详情内容中
    // （对话框内只有 ip_address 输入，不会提前出现该值，避免对话框旧 DOM
    // 造成的误判）。
    await waitForUi(() => {
    ...
    expect(wrapper.text()).toContain('2026-09-18T10:00:00Z')
    // 请求序列：初始 GET → PATCH → 重新读取 GET。
    expect(fetchMock).toHaveBeenCalledTimes(3)
    expect(fetchMock.mock.calls[2]![0]).toBe('/api/ip-addresses/41')
  })
  ...
  it('409 CONFLICT + DUPLICATE → 「该 IP 在所属 Cluster 内已被占用」（不解析 message）', async () => {
    stubDetailFetch({
      detail: () => jsonResponse(200, IP_ADDRESS_A),
      patch: () =>
        jsonResponse(409, {
          error: {
            ...
            message: '与展示无关的冲突文案',
            details: [
              { field: 'ip_address', code: 'DUPLICATE', message: '与展示无关的重复提示' },
            ],
          },
        }),
    })
    ...
    await wrapper.find('[data-testid="ip-form-ip-address"]').setValue('10.0.0.10')
    await wrapper.find('[data-testid="ip-form-submit"]').trigger('click')
  ...
  it('提交中：提交按钮 Loading，重复点击不产生第二个 PATCH（禁止重复提交）', async () => {
    ...
    await wrapper.find('[data-testid="ip-form-ip-address"]').setValue('10.0.1.9/16')
    await wrapper.find('[data-testid="ip-form-submit"]').trigger('click')
    ...
    // 等待重新读取完成：新 updated_at 只会出现在重新读取后的详情内容中。
    await waitForUi(() => {
      expect(wrapper.text()).toContain('2026-09-18T13:00:00Z')
    })
    expect(wrapper.text()).toContain('10.0.1.9/16')
  })
})
...
describe('IpAddressDetailPage 删除入口（T-41 / AC-23）', () => {
  ...
  it('未通过二次确认（确认框取消）→ 不发送 DELETE 请求', async () => {
    const fetchMock = stubDetailFetch({ detail: () => jsonResponse(200, IP_ADDRESS_A) })
