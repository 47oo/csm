...
/** 登记表单所需宿主裸金属选项（GET /api/bare-metals，f002 契约 §3.2 信封）。 */
const BARE_METAL_LIST_BODY = {
  items: [
    {
      id: 3,
      cluster_id: 1,
...
/** 按端点 / 方法分发的 fetch 桩（登记对话框会额外请求 GET /api/bare-metals）。 */
...
async function selectHost(wrapper: VueWrapper, bareMetalId: number): Promise<void> {
...
describe('VirtualMachineListPage 登记入口（T-FE-01 / AC-01~08，§21 不预判）', () => {
  ...
  it('空 name 仍提交（契约 §7 未定义约束：前端不得基于空串编写业务分支）', async () => {
    const fetchMock = stubCreateFetch({
      list: () => jsonResponse(200, LIST_BODY),
