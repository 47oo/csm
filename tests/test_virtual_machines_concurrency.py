"""F006 并发 / 锁序测试（T-24、T-25，数据库层，真实 PostgreSQL）。

使用真实 SQLAlchemy 会话直接驱动服务函数（``create_virtual_machine`` /
``delete_bare_metal``），在提交前精确控制事务持锁时刻，验证：

- 创建 VM 对宿主 BareMetal 行取**共享锁**（``FOR SHARE``），与删除的
  ``FOR UPDATE`` 互斥；
- 两种交错（先建后删 / 先删后建）结束后，孤立记录不变式恒为 **0 行**
  （不存在「宿主已删 + VM 活跃」；ADR-0004 §5 / AC-26）。
"""
...
# --------------------------------------------------------------------------- #
# T-24 / AC-26：并发「创建 VM vs 删除宿主」孤立记录不变式 = 0
# --------------------------------------------------------------------------- #
def test_t24_create_commits_first_delete_sees_active_child(concurrency_env):
    ...
    try:
        ...
        virtual_machine_service.create_virtual_machine(
            create_session,
            VirtualMachineCreate(bare_metal_id=host_id, name="vm1"),
        )
        ...
        assert conn.execute("SELECT count(*) FROM virtual_machines").fetchone()[0] == 1
    finally:
        delete_session.close()


def test_t24_delete_commits_first_create_is_rejected(concurrency_env):
    _, factory, conn = concurrency_env
    host_id = _insert_host(conn, "cluster-a")
    ...
    try:
        ...
        assert _invariant(conn) == 0
        assert conn.execute("SELECT count(*) FROM virtual_machines").fetchone()[0] == 0
    finally:
        create_session.close()


# --------------------------------------------------------------------------- #
# T-25 / AC-27：创建对宿主行取共享锁并在同一事务内确认活跃
# --------------------------------------------------------------------------- #
def test_t25_create_blocks_on_host_exclusive_lock_then_rejects(concurrency_env):
    _, factory, conn = concurrency_env
    host_id = _insert_host(conn, "cluster-a")
    ...
    def do_create() -> None:
        try:
            virtual_machine_service.create_virtual_machine(
                session, VirtualMachineCreate(bare_metal_id=host_id, name="vm1")
            )
            session.commit()
            outcome["create"] = "ok"
        except NotFoundError:
    ...
    thread.join(timeout=15)
    assert not thread.is_alive()
    # 宿主删提交后重新求值 → 未命中活跃宿主 → 拒绝创建，不留无主 VM
    assert outcome["create"] == "not_found", outcome
    assert conn.execute("SELECT count(*) FROM virtual_machines").fetchone()[0] == 0
    assert _invariant(conn) == 0
    session.close()
...
def test_t25_host_gate_uses_row_lock(concurrency_env):
